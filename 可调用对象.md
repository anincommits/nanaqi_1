...
仿函数与[[模板]]相结合，可以实现高度通用和可复用的代码。例如，编写一个通用的比较仿函数。
```CPP
#include <iostream>
#include <vector>
#include <algorithm>
...
`std::function` 是C++11提供的一个通用的可调用包装器，能够封装**任何可调用对象，包括普通函数、Lambda表达式、函数对象以及绑定表达式。实现了类型擦除，使得不同类型的可调用对象可以通过统一的接口进行操作。**
```CPP
#include <iostream>
#include <functional>
